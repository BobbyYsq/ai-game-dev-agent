...
if (-not (Test-Path $MambaExe)) { $ArchivePath = Join-Path $MambaDir "micromamba.tar.bz2"; Invoke-WebRequest -Uri "https://micro.mamba.pm/api/micromamba/win-64/latest" -OutFile $ArchivePath; tar -xf $ArchivePath -C $MambaDir; Copy-Item (Get-ChildItem $MambaDir -Recurse -Filter micromamba.exe | Select-Object -First 1).FullName $MambaExe -Force }
