...
- The task prompt injects only a capability registry, skill metadata listing, Godot docs index, and runtime summary by default. Full skill bodies and Godot docs excerpts are loaded only through explicit skill use or LLM `context_requests`.
- Automatically matched skills contribute only lightweight metadata until the user explicitly invokes `/skill` or the LLM requests the full skill body through `context_requests`.
